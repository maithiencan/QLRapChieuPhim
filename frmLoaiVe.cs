...
{
    public partial class pLoaiVe : Form
    {
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (txtMaLV.Text.Trim() == "")
                ...
                MessageBox.Show("Mã loại vé phải nhập đúng 10 ký tự!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            else
            {
                try
                {
                    if (maLV == "")
                    {
                        ...
                        SqlCommand cmd = new SqlCommand(sql);
                        cmd.Parameters.Add("@MALV", SqlDbType.NVarChar, 10).Value = txtMaLV.Text.ToUpper();
                        cmd.Parameters.Add("@TENLV", SqlDbType.NVarChar, 20).Value = txtTenLV.Text;
